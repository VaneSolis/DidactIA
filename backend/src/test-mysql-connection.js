...
async function testConnection() {
  ...
  try {
    ...
    connection = await mysql.createConnection({
      ...
      database: process.env.DB_NAME || 'didactiabd'
    });
    ...
    const dbExists = databases.some(db => Object.values(db)[0] === (process.env.DB_NAME || 'didactiabd'));
    
    if (dbExists) {
      console.log('✅ Base de datos "didactiabd" existe');
      ...
      console.log('❌ Base de datos "didactiabd" no existe');
      console.log('💡 Crea la base de datos en MySQL Workbench:');
      console.log('   CREATE DATABASE didactiabd;');
    }
    ...
    if (error.code === 'ER_ACCESS_DENIED_ERROR') {
      ...
      console.log('   - Crea la base de datos: CREATE DATABASE didactiabd;');
    }
  } finally {
    if (connection) {
